...
# Parameters
K = 36 # number of umbrellas
N_max = 4001 # maximum number of snapshots/simulation
...
atom_indices = np.asarray([[0,4,6,8]])
prmtop = "watVA.prmtop"
...
for k in range(K):
    ...
    trajfile = "umbrella%i.dcd"%k
    traj = md.load(trajfile, top=prmtop)
    traj_noeq = traj[equil:]
    print("Discarding %i frames for equilibration..."%equil)
    print("Computing torsion angles for  %s..." % trajfile)
    dihedrals = md.compute_dihedrals(traj_noeq,atom_indices)
    # Parse data.
    n = 0
    for val in dihedrals:
        chi_kn[k,n] = val*180/math.pi
        n+=1
    N_k[k] = n
